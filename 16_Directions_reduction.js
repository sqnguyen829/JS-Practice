function dirReduc(arr){
    ...
    for(let i=0; i<arr.length; i++){
      if(i+1 < arr.length){
        if(checkDirection(arr[i], arr[i+1])){
          return dirReduc(newArr.concat(arr.slice(i+2)))
        }else{
          newArr.push(arr[i])
        }
      }else{
  ...
  function checkDirection(d1, d2){
    ...
    return d2.toLowerCase() == check[`${d1.toLowerCase()}`] ? true : false
  }
